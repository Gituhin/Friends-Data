...
def verify_mail(mail):
    ...
    <br>I carried you the verification code from email servers.</p>

    <p>Here is your E-mail verification code: <b>{code}</b><br>
    Enter this code in the app.<br><br>Regards,<br>
    Zeng the Goose (Context: Kung Fu Panda).
...
def push_to_github(by_name):
    ...
    repo.update_file(file.path, "Commit by "+str(by_name), text1, file.sha, branch="main")
...
def make_edits(data, val_list, idx):
    ...
    by_name = data.loc[idx, 'Name']
    push_to_github(by_name)
...
def push_new(data, val_list):
    ...
    data.to_csv("database.csv", index=False) #updating to local storage
    by_name = data.loc[len(data)-1, 'Name']
    push_to_github(by_name)